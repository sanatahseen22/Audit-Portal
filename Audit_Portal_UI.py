...
# Define the path for the "Input.xlsx" file in the "Audit Portal" folder on the user's desktop
desktop_path = os.path.join(os.path.expanduser("~"), "Desktop", "Audit Portal", "Input.xlsx")

# Load the input data from the dynamically located file
input_data = pd.read_excel(desktop_path)
...
# Function to save data directly into the dynamically located input file
def save_row(index):
    ...
    # Save the updated input_data back to the dynamically located input file
    input_data.to_excel(desktop_path, index=False)

    st.success(f"Row {index+1}: Data saved successfully to the input file!")
